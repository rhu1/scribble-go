...
func (ini *Server_1To1_Init) Accept(rolename string, id int, conn transport.Transport) error {
...
func (ini *Worker_1Ton_Init) Connect(rolename string, id int, conn transport.Transport) error {
	cn, ok := ini.ept.Conn[rolename]
	if !ok {
		return fmt.Errorf("rolename '%s' does not exist", rolename)
	}
