...
type Server_1Ton_Init struct {
	session.LinearResource
	ept *session.Endpoint
}

func NewServer(id, nserver, nworker int) (*Server_1Ton_Init, error) {
	if id > nserver || id < 1 {
...
type Server_1Ton_End struct {
}

// Session has started, so if an error occurs, then a runtime error is produced
// and the program exits
func (st1 *Server_1Ton_1) SendAll(pl []int) *Server_1Ton_1 {
...
func (ini *Server_1Ton_Init) Run(f func(*Server_1Ton_1) *Server_1Ton_End) {
