//rhu@HZHL4 ~/code/go
//$ go install github.com/rhu1/scribble-go-runtime/test/foo/foo4
//$ bin/foo4.exe
...
import (
	...
	"github.com/rhu1/scribble-go-runtime/test/foo/foo4/Foo4/Proto1"
...
func serverCode(wg *sync.WaitGroup, n int) *Proto1.Proto1_S_1To1_End {
	...
	var x int
	if 1 < 2 {
		end = s2.Split_W_1Ton_b(1234, util.Copy).Reduce_W_1Ton_c(&x, util.Sum)
		fmt.Println("S got c:", x)
	} else {
		end = s2.Split_W_1Ton_d(5678, util.Copy)
	}
...
func clientCode(wg *sync.WaitGroup, n int, self int) *Proto1.Proto1_W_1Ton_End {
	...
	case w3 := <-w2.Recv_S_1To1_b(&x):
		...
		end = w3.Split_S_1To1_c(5678, util.Copy)
