...
func server_WK(wg *sync.WaitGroup, K int, self int) *WK.End {
	...
	defer ss.Close()
	// family 1 -- K > 3 -- so must accept from W3toKsub1
...
func server_W3toKsub1(wg *sync.WaitGroup, K int, self int) *W3toKsub1.End {
	...
	if self == 3 {
		...
		if err = M.W_3toK_not_1to1and2to2and2toKsub1_Accept(self-1, ss, FORMATTER()); err != nil {
	...
	if self == K-1 {
		...
		if err := M.W_2to2and2toKsub1_not_1to1and3toK_Dial(self+1, util.LOCALHOST, PORT+self+1, DIAL, FORMATTER()); err != nil {
